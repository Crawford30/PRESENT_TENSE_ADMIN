...
class ImportVariousTonguesSongsRequest extends FormRequest
{
    ...
    private function saveData(array $formattedContent)
    {
        ...
        foreach ($formattedContent as $item) {
           ...
           $data = [
            ...
            $presentTenseSongData = SongsInVariousTongues::updateOrCreate([
                'song_number' => $item['song_number'],
                // 'song_title' => $item['song_title'],
                // 'song_body' => $item['song_body'],
            ], $data);
        ...
        return apiResponse(SongsInVariousTongues::all());
