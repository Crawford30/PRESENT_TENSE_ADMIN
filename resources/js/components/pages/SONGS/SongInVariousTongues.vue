    ...
    <div class="container">
        <div class="row my-3">
            <div class="col-md-9 text-left">
                <h4>Songs In Various Tongues</h4>
            </div>
            <div class="col-md-3">
                <div class="btn-group add-new-dropdown" style="width: 100%">
                    <div class="btn-group w-100" role="group">
                        <button type="button" class="btn btn-primary" @click="showAddSingleSongModal">
                            Add Song
                        </button>
                        <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <div class="dropdown-menu">
                                <div class="border-bottom dropdown-item text-center" @click="showAddSingleSongModal">
                                    <img src="/images/icons/user_folder.png" alt="single image" class="img-logo" />
                                    <p class="font-weight-light small mt-1">Single Record</p>
                                </div>
                                <div class="border-bottom dropdown-item text-center" @click="showUploadExcel">
                                    <img src="/images/icons/upload_purple.png" alt="single image" class="img-logo" />
                                    <p class="font-weight-light small mt-1">Batch Upload</p>
                                </div>

                                <a class="pt-2 mb-0 dropdown-item text-center" @click="downloadTemplate">
                                    <img src="/images/icons/icon.excel.png" alt="single image" class="img-logo" style="height: 25px !important" />
            ...
            <div class="staff-card shadow-sm table-padding">
                <div class="row justify-content-center">
                    <table class="table table-sm present-tense-table">
                        ...
                        <tr v-for="(variousTongueSong, index) in variousTongueSongs.results" :key="variousTongueSong.id + '_' + index">
                            <td>{{ index + 1 }}</td>
                            <td>{{ variousTongueSong.song_number }}</td>
                            <td class="text-left">
                                {{ variousTongueSong.song_title }}
                            </td>
                            <td>{{ variousTongueSong.created_at | myDate }}</td>
                            <td>
                                <a href="#" @click="updateSong(variousTongueSong)">
                                    <i class="fas fa-pencil-alt" style="color: #999; font-size: 18px"></i>
                                </a>

                                <a href="#" @click="deleteSong(variousTongueSong.id)" style="margin-left: 8px">
                                    <i class="far fa-trash-alt" style="color: #999; font-size: 18px"></i>
                                </a>

                                <a href="#" @click="viewSongDetail(variousTongueSong)" style="margin-left: 8px">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body p-4">
                    <button type="button" style="position: absolute; right: 1.5rem; top: 1.5rem" class="close" @click="closeModel">
                        &times;
                    </button>
                    <div v-show="!editmode">
                    ...
                    <form id="song-form" ref="songRef">
                        <input hidden name="id" v-if="selectedSong != null" :value="selectedSong.id" />

                        <div class="form-group">
                            <label>Song Number</label>
                            <input v-if="selectedSong != null" :value="selectedSong.song_number" name="song_number" type="text" class="form-control" autocomplete="off" placeholder="Song Number" required />
                            <input v-else v-model="songNumber" name="song_number" type="text" class="form-control" autocomplete="off" placeholder="Song Number" required />
                        </div>
                        <!-- <p style="color: red">Song number is required</p> -->

                        <div class="form-group">
                            <label>Song Title</label>
                            <vue-editor v-if="selectedSong != null" id="song-title" placeholder="Please Type Song Title Here" :disabled="false" v-model="songTitle" :editorToolbar="defaultToolbar"></vue-editor>

                            <vue-editor v-else id="song-title" placeholder="Please Type Song Title Here" :disabled="false" v-model="songTitle" :editorToolbar="defaultToolbar"></vue-editor>
                        </div>

                        <div class="form-group">
                            <label>Song Body</label>
                            <vue-editor id="song-body" placeholder="Please Type Song Body Here" :disabled="false" v-model="songBody" :editorToolbar="defaultToolbar"></vue-editor>
                        </div>
                    </form>
                    ...
                    <div class="row d-flex justify-content-center">
                        ...
                        <div class="col-4">
                            <div class="text-center mt-2">
                                <button :disabled="isProcessing" type="button" class="present-tense-btn present-tense-primary" @click="saveSong">
            ...
            <div class="modal-content">
                <div class="modal-body p-4">
                    ...
                    <form id="song-form" ref="songRef">
                        <div class="form-group">
                            <!-- <label>Song</label> -->
                            <vue-editor id="song-body" :disabled="true" v-model="songBody" :editorToolbar="defaultToolbar"></vue-editor>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body py-4">
                      ...
                      ">
                                    ...
                                    <div class="position-relative">
                                        <button type="button" class="btn btn-primary position-relative" style="font-size: 12px; margin-top: 5px">
                                            Browse
                                            <input @change="onBrowseFile" type="file" style="
                            position: absolute;
                            left: 0;
                            top: 0;
                            opacity: 0;
                    ...
                    <div class="text-center mt-2">
                        <button :disabled="file === null || isProcessing" @click="uploadFile" type="button" class="present-tense-btn present-tense-primary">
                            <span>
    ...
    methods: {
        saveSong() {
            ...
            if (form.valid()) {
                ...
                axios({
                        method: "post",
                        url: "/api/various-tongue/create-various-tongue-song",
                        data: songFormData
                        //form.serialize(),
                    })
                    .then(response => {
        ...
        deleteSong(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "api/various-tongue/delete-various-tongue-song",
                        type: "post",
                        data: {
                            various_tongue_song_id: id
                        },
                        success(data) {
                            Swal.fire(
        ...
        geBigSongBookSongs() {
            let app = this;
            axios
                .get("api/various-tongue/list")
                .then(response => {
                    app.variousTongueSongs = response.data;
                })
                .catch(error => {
        ...
        uploadFile() {
            ...
            axios({
                    method: "post",
                    url: "api/various-tongue/import-various-tongue-songs",
                    data: formData,
                    headers: {
